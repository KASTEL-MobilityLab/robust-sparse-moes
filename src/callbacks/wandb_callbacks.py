...
from src.models.attack_module import _AttackLitModule
...
def except_on_sanity_check(func):
    @wraps(func)
    def wrapped_func(self, trainer, *args, **kwargs):
...
class WatchModel(Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        self.log = "gradients"
        logger.watch(
            model=trainer.model.model, log=self.log, log_freq=self.log_freq, log_graph=True
        )
...
class LogConfusionMatrix(Callback):
    ...
    @except_on_sanity_check
    def on_validation_batch_end(
        self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx
    ):
        """Gather data from single batch."""
        self.probs.append(outputs[""])
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        wandb.log({"confusion_matrix": wandb.plot.confusion_matrix(probs, preds, targets)})
...
class LogImagePredictions(Callback):
    ...
    @except_on_sanity_check
    def log_prediction_masks(self, trainer, pl_module: LightningModule, dataloader, name):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        self.log_prediction_masks(trainer, pl_module, trainer.datamodule.val_dataloader(), "val")

    def on_test_epoch_end(self, trainer, pl_module):
        self.log_prediction_masks(
            pl_module,
            trainer.datamodule.test_dataloader(),
            "attack" if isinstance(pl_module, _AttackLitModule) else "test",
        )
